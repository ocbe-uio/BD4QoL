...
if (variables == 'complete'):
    ...
    model_folder ='reduced_' + model_type 
...
if os.path.isdir('results/'+model_folder):
    print(f"Warning: the directory results/{model_folder} already exists!")
    model_folder = model_folder  + '_2'
    print(f"Results will be saved in results/{model_folder} instead.")

# create directory
os.mkdir('results/'+model_folder)

variables_list_file = "data/prospective_variables.csv"

print(f"Modelling: hn4_dv_c30_ghs decline")
print('Variables were selected from: '+variables_list_file)
print('Results will be saved at results/'+model_folder)
...
# define labels
df_orig = df_orig.assign(label = np.where(df_orig[outcome]<=df_orig[baseline]-10, 1, 0))
...
# save model
with open('results/' + model_folder + '/model_orig.pkl', 'wb') as model_file:
...
for bootstrap in range(0, n_bootstraps):
    ...
    bs_n = bs_n.assign(label = np.where(bs_n[outcome] <= bs_n[baseline]-10, 1, 0))
    ...
    # save model
    with open('results/' + model_folder + '/model_bs' + str(bootstrap + 1) + '.pkl', 'wb') as model_file:
        pickle.dump(pipeline, model_file)